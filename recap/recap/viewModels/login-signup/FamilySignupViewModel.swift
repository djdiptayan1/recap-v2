...
class FamilySignupViewModel: ObservableObject {
    ...
    func completeDetails() {
        guard !name.isEmpty, !phone.isEmpty, !relation.isEmpty else {
    ...
    func finalizeSignup() {
        ...
        Task {
            do {
                ...
                let response = try await FamilySignupService.shared.createFamilyProfile(request: request)
                
                guard response.success, let data = response.data else {
                    throw NSError(domain: "Signup", code: 0, userInfo: [NSLocalizedDescriptionKey: response.message])
                }
