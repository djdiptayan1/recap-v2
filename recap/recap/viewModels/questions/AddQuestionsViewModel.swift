...
class AddQuestionsViewModel: ObservableObject {
    ...
    let categories = ["Immediate Memory", "Recent Memory", "Remote Memory"]
    ...
    func submitQuestion(patientID: String) async {
        ...
        // Map display category to backend key
        let backendCategory: String
        switch category {
        case "Immediate Memory": backendCategory = "immediateMemory"
        case "Recent Memory": backendCategory = "recentMemory"
        case "Remote Memory": backendCategory = "remoteMemory"
        default: backendCategory = "immediateMemory"
        }
        ...
        do {
            let _: AddQuestionResponse = try await NetworkManager.shared.request(
                endpoint: QuestionsAPI.addQuestion(patientID: patientID, data: request))
